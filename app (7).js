(function(){
  ...
  function render(){
    ...
    goal.steps.forEach(s => {
      ...
      else if(s.status === 'active'){ fill = 'var(--clay)'; extraClass = 'pulse'; r = 19; }
  ...
  function renderTaskList(goal, pos, inc){
    ...
    title.textContent = 'What\'s left to reach the summit';
    wrap.appendChild(title);

    remaining.forEach(s => {
      const row = document.createElement('div');
      row.className = 'taskItem ' + s.status;
      row.addEventListener('click', () => onStepClick(s.id));
